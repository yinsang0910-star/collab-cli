...
> 将本文件内容合并到项目的 `.reasonix/system.md` 或 `reasonix.toml` 的 `[system].instructions` 中。
> 本协议定义了 Reasonix 在本项目中必须遵守的协作规则。
...
```
Step 1: read_file .shared/MANIFEST.md
        → 获取系统规则、agent 注册表、角色定义

Step 2: read_file .shared/SHARD.md
        → 获取项目当前状态（≤80行，30秒内读完）

Step 3: read_file .shared/BADGE-{your-id}.md
        → 存在且 session 有效？→ 确认身份，继续
        → 不存在？→ 向用户申请工牌，说明你的能力和建议角色
        → 没有总工？→ 第一个申请的 agent 可自荐为总工（L4），由用户确认

Step 4: ls .shared/inbox/{your-id}/ + grep "status: unread" 过滤未读消息
        → 有 P0/P1 未读？→ 在回复用户之前先告知并询问是否优先处理

Step 5: grep "assignee: {your-id}" .shared/tasks/ 找到自己的活跃任务
        → 有 IN_PROGRESS 任务？→ 列出，提醒用户
...
|------|------|-----------|
| L0 | 观察者 | 只读，不能改任何文件 |
| L1 | 执行者 | write_file 自己的任务文件 + 写 inbox |
| L2 | 贡献者 | L1 + write_file memory + 提交审查 |
| L3 | 审查者 | L2 + 审批任务 + write_file SHARD |
...
| 文件 | 谁能写 | 规则 |
|------|--------|------|
| SHARD.md | L3+ | 写前 read_file 检查 `last_updated_by`，避免覆盖他人更新 |
...
- memory/ 下每个文件 ≤50 行，超限时用 bash 运行 `collab memory compact`
- 旧的"最近完成"记录（>3天）自动归入 archive/

## CLI 工具

本项目已安装 `collab` CLI。常用命令（通过 bash 工具执行）：

```bash
collab status              # 查看全局状态
collab inbox check {id}    # 检查未读消息
collab task list           # 列出活跃任务
collab badge show {id}     # 查看当前工牌
collab memory compact      # 触发记忆归档
collab handshake {id}      # 完整握手检查
collab heartbeat {id} --once  # 心跳检查（单次）
```

也可以直接用 read_file / write_file / edit_file 操作 `.shared/` 下的文件。
...
1. 重新 read_file 文件
2. 尝试用 edit_file 合并你的改动
3. 如果无法自动合并，write_file 到 `conflicts/C-{timestamp}.md` 等总工仲裁
