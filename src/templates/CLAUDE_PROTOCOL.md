...
**在你对用户的每条消息做出回应之前**，检查是否有新的 inbox 通知：

1. 读 `.claude/inbox-notification.md`
2. 如果文件存在且内容非空 → 先告知用户有新消息，询问是否优先处理
3. 处理完后清空通知文件（写入空内容）

**这是单机模式下的"准实时"通知机制。** 其他 agent 发消息给你时，系统会自动在 `.claude/inbox-notification.md` 写入通知摘要。你每次响应前读一下就能及时发现。

## 当前对话中

- **不需要开新对话**才能查看 inbox 或任务
- 用户说"检查 inbox"时，运行 `collab inbox check claude-01`
- 用户说"看看任务"时，运行 `collab task list`
- WorkBuddy 发来的消息，用户可以直接告诉你"WB 给你发消息了"，你去读就行
